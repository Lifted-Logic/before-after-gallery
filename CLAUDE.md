...
**Adding a new markup filter hook:** Add a `public static function` to `src/Hooks/Hooks.php` that builds `$markup` and returns `apply_filters('lifted_logic/bag/{name}', $markup, ...$parts)`. Call it as `Hooks::method_name()` in the template. Document it in `README.md` under the Hooks section. For a new plain link/CTA button, compose it from `Hooks::bag_primary_button_markup()` or `Hooks::bag_secondary_button_markup()` (see below) rather than writing new heredoc markup, then wrap the result in your own named filter so it stays individually overridable. Current hooks: `bag_back_button_markup`, `bag_related_slider_arrows_markup`, `bag_primary_button_markup`, `bag_secondary_button_markup`, `bag_link_card_markup`, `bag_nsfw_modal_markup`, `bag_filter_actions_markup`, `bag_hero_banner_link_markup`, `bag_categories_all_link_markup`, `bag_grid_view_all_link_markup`, `bag_related_bna_link_markup`.

**Shared button generators:** `bag_primary_button_markup()` and `bag_secondary_button_markup()` are the single source of truth for every plain `<a>` CTA button's markup — solid `ba_btn-primary` buttons and icon-flanked `ba_btn-secondary` buttons, respectively. Each fires its own plugin-wide filter (`lifted_logic/bag/primary_button_markup` / `.../secondary_button_markup`) before the calling method's own named filter fires, so a theme can restyle every button of one style in a single hook, or override just one instance via its existing per-instance filter, without the two conflicting. `bag_back_button_markup()` is a secondary-button variant that overrides `base_class` (full replace, not append) and `icon` (`arrow-left` instead of `arrow-right`).
